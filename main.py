...
def load_rfdetr_model(cpkt: str):
    try:
        ...
        if args and hasattr(args, "resolution") and hasattr(args, "hidden_dim"):
            ...
            print(f"Detected resolution: {resolution}, hidden_dim: {hidden_dim}")
            
            # Mapping resolusi ke model
            if resolution == 384:
                print("Assuming the checkpoint corresponds to RF-DETR Nano")
                return RFDETRNano(pretrain_weights=cpkt)
            elif resolution == 432:
                # Custom resolution - kemungkinan model segmentasi atau custom training
                print("Detected custom resolution 432 - Loading as RF-DETR Segmentation Preview")
                ...
                # Fallback untuk resolusi yang tidak dikenali
                print(f"Warning: Unknown resolution {resolution}, attempting to load as segmentation model")
                return RFDETRSegPreview(pretrain_weights=cpkt)
        ...
        # Instance segmentation model
            ...
            if len(obj["model"]) == 544:
                ...
                print(f"Detected model with {len(obj['model'])} parameters, attempting to load as segmentation model")
                return RFDETRSegPreview(pretrain_weights=cpkt)
...
def export_onnx(checkpoint: str, onnx_name: Optional[str] = None, no_simplify: bool = False) -> None:
    ...
    try:
        ...
        # Use legacy ONNX export API to avoid torch.export issues
        import os
        ...
        torch.onnx.export(
            ...
            verbose=False,
            # Use JIT tracing instead of torch.export
            dynamo=False
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-simplify",
        action="store_true",
        help="Do not simplify the ONNX model (disabled by default)",
    )
